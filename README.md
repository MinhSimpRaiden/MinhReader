...
- Cách thêm plugin bằng URL:
  1. Vào `Nguồn truyện`.
  2. Vào `Plugin`.
  3. Bấm `+ Thêm plugin`.
  4. Chọn `Dán URL plugin`.
  5. Dán URL trỏ tới file manifest JSON thô (raw JSON).
  6. Bấm `Tải plugin`.
  7. Xem preview an toàn (kèm URL nguồn).
  8. Bấm `Cài plugin`.
- Chỉ cài plugin từ nguồn tin cậy. App chỉ nhận JSON manifest, không chạy code từ plugin.
